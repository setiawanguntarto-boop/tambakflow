...
const WhatsAppMockup = () => {
  ...
  const messages = [{
    type: "received",
    text: "Selamat pagi! Mau lapor untuk tambak mana?\n1. Tambak Mutiara - A1\n2. Jaya Makmur - A2\n3. Bintang Timur - B1",
    time: "08:01"
  }, {
    type: "sent",
    text: "2",
    ...
    type: "received",
    text: "Tambak A2. Silakan isi data:\n1. pH Pagi:\n2. Pakan (kg):\n3. Salinitas (ppt):\n4. Kecerahan (cm):\n5. Catatan:",
    time: "08:02"
  }, {
    type: "sent",
    text: "1. 6.9\n2. 18\n3. 25\n4. 30\n5. Nafsu makan udang sedikit menurun, air agak keruh.",
    time: "08:05"
  }, {
    type: "received",
    text: "Oke, data diterima. Terima kasih! Laporan untuk Jaya Makmur - A2 telah diperbarui di dasbor.",
    time: "08:06"
  }];
  useEffect(() => {
  ...
  return <div className="bg-card rounded-2xl shadow-lg p-6 sticky top-8">
      ...
      <div className="w-full max-w-sm mx-auto h-[700px] bg-white rounded-[2.5rem] p-1 border-2 border-gray-800 shadow-[0_0_0_2px_#000,0_0_0_6px_#333,0_20px_40px_rgba(0,0,0,0.4)] flex flex-col relative overflow-hidden" style={{
      background: 'linear-gradient(145deg, #1f1f1f 0%, #2d2d2d 50%, #1a1a1a 100%)'
        ...
        {/* Chat Area */}
        <div className="flex-1 p-3 overflow-y-auto flex flex-col gap-2 my-2 -mx-2" style={{
          ...
          {messages.slice(0, currentMessageIndex).map((message, index) => <div key={index} className={`max-w-[85%] p-2 rounded-lg text-sm animate-in slide-in-from-bottom-2 duration-500 ${message.type === "sent" ? "bg-whatsapp-sent self-end" : "bg-whatsapp-bubble self-start shadow-sm"}`}>
              <span className="break-words whitespace-pre-line">
                {message.text.includes("6.9") ? message.text.split("6.9").map((part, i) => <span key={i}>
                      {part}
                      {i === 0 && <span className="font-bold text-destructive">6.9</span>}
                    </span>) : message.text.includes("Jaya Makmur - A2") ? message.text.split("Jaya Makmur - A2").map((part, i) => <span key={i}>
                      {part}
        ...
        <div className="p-3 bg-[#f0f0f0] flex items-center flex-shrink-0 rounded-b-[2rem] border-t border-gray-200">
          ...
          <input type="text" placeholder="Ketik pesan..." className="flex-1 mx-2 px-4 py-2 bg-white border border-gray-300 rounded-full text-sm" disabled />
